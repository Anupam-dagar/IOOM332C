...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (testcases-- > 0)
        {
            ...
            int diningColumn;
            Person[][] diningArray;
            Person[] diningSingleArray;
            int[][] diningFlagArray;
            int[] diningSingleFlagArray;
            for (int i = 0; i < a; i++)
            {
            ...
            int counter;
            diningRow = scan.nextInt();
            diningColumn = scan.nextInt();
            diningArray = new Person[diningRow][diningColumn];
            diningSingleArray = new Person[sizeOfArray];
            diningFlagArray = new int[diningRow][diningColumn];
            diningSingleFlagArray = new int[sizeOfArray];
            counter = a;
            ...
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    {
                        diningSingleArray[counter] = person[i][j];
                        counter = counter + 1;
                    }
                }
            }
            r = 1;
            receptionist = new Receptionist[r];
            Fcfsrecpetionist fcfsrecpetionist = new Fcfsrecpetionist("first", "R00");
            receptionist[0] = fcfsrecpetionist;
            insert.insertFirst(receptionist, diningFlagArray, diningSingleFlagArray, diningArray, diningSingleArray, diningRow, diningColumn, counter);
            for (int i = 0; i < m; i++)
            {
                for (int j = 0; j < n; j++)
                {
            ...
            for (int i = 0; i < diningRow; i++)
            {
                for (int j = 0; j < diningColumn; j++)
                {
                    if (diningFlagArray[i][j]==1)
                    {
                       diningArray[i][j].print();
                    }
                }
            }
        }
